<?php
 ...
 $app->get('/add/{n1}/{n2}', function ($n1, $n2) use($app) {
    return '<h2 style = "font-family: Arial" >Сумма: </h2><h3>' . $n1 . ' + ' . $n2 .' = ' .($n1 + $n2) . '</h3>'; 
  })->after($hdrs);
 ...
 $app->get('/sub/{n1}/{n2}', function ($n1, $n2) use($app) {
    return '<h2>Вычитание: </h2><h3>' . $n1 . ' / ' . $n2 .' = ' .($n1 / $n2) . '</h3>'; 
  })->after($hdrs);
 ...
 $app->get('/pow/{n1}/{n2}', function ($n1, $n2) use($app) {
    return '<h2>Возведение в степень: </h2><h3>' . $n1 . ' ^ ' . $n2 .' = ' .($n1 ** $n2) . '</h3>'; 
  })->after($hdrs);

 $app->get('/author', function () use($app) {
    return '<h4 id="author" title="GossJS">Skorobogatov Kirill</h4>'; 
  })->after($hdrs);
